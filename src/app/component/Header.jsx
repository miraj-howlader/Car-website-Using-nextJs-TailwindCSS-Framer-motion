...
const Header = () => {
  ...
  return (
    <header className={`${header ?" bg-white shadow-md py-3":"bg-transparent shadow-none py-4"} fixed w-full max-w-[1920px]
      mx-auto z-20 transition-all duration-300`}>

      <div className="xl:container mx-auto flex flex-col
       xl:flex-row xl:items-center xl:justify-between">
        <div className=" flex justify-between items-center px-4">
          {/* logo  */}
          <Link to="home" smooth={desktopMode} spy={true}
          className=" cursor-pointer">
          <Image src={'/icons/logo.svg'} width={174} height={64} alt=""/>
          </Link>
          {/* nav open menu  */}
          <div 
          className=" cursor-pointer xl:hidden"
          onClick={()=>setNav(!nav)}>
            {nav ? <RxCross2 className=" text-4xl"/>:<CiMenuBurger className=" text-4xl"/>}
          </div>
        </div>
        {/* nav  */}
